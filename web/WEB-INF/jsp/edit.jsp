...
<%@ page import="com.sun.xml.bind.v2.schemagen.xmlschema.List" %>
        ...
        <c:forEach var="type" items="<%=SectionType.values()%>">
            ...
            <c:choose>
                <c:when test="${type.name() == SectionType.PERSONAL || type.name() == SectionType.OBJECTIVE}">
                    <input type="text" name="section" size="100" value="${resume.getSection(type)}">
                </c:when>
                <c:when test="${type.name() == SectionType.ACHIEVEMENT || type.name() == SectionType.QUALIFICATIONS}">
                    ...
                    <c:forEach var="item" items="${content.items}">
                        <textarea cols="100" name="sectionItem" rows="4">${item}</textarea>
                    </c:forEach>
                </c:when>
            </c:choose>
        </c:forEach>
